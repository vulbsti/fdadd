...
import { ensureAtrosInstalled } from './atros-commands';
...
export async function preparePiWorkspace(runId: string) {
  ...
  const identity = createHash('sha256').update(`${authority.userId}:${authority.personId}:${runId}:${authority.modeEpoch}:${authority.privacyEpoch}:${authority.birthRevision}:${bundle}`).digest('hex').slice(0, 32);
  const sandbox = await Sandbox.getOrCreate({
    name: `aidoraa-pi-${identity}`, runtime: 'node22', timeout: 20 * 60 * 1000, persistent: false,
    onCreate: async (fresh) => {
      await fresh.writeFiles(runtimeFiles);
      const install = await fresh.runCommand('npm', ['ci', '--ignore-scripts', '--prefix', RUNTIME], { timeoutMs: 240000 });
      if (install.exitCode !== 0) throw new Error('Pinned Pi installation failed.');
      if (loaded.birth) await ensureAtrosInstalled(fresh);
    },
  });
  ...
  const alreadyPrepared = await sandbox.readFileToBuffer({ path: configPath });
  if (alreadyPrepared) return { sandboxName: sandbox.name, configPath, authority };
