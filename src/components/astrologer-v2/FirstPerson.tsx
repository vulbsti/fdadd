...
export default function FirstPerson() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!response.ok || !body?.profileId) {
      setError(body?.message ?? 'Could not create this person.'); setSaving(false); return;
    }
  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-[#fbfaf6] p-5 text-[#112d52]">
      <form onSubmit={submit} className="w-full max-w-xl rounded-lg border border-[#ded9d0] bg-white/45 p-8 shadow-sm md:p-12">
        ...
        <h1 className="mt-3 font-serif text-5xl leading-[1.08]">Whose life are we understanding?</h1>
