-- A typed change must not strand older accepted sources. Privacy-changing
-- corrections cancel older pending/running work by epoch, so the replacement
-- job owns the entire still-unpublished source interval through this change.
...
begin
  ...
  insert into public.person_jobs (
    ...
    v_head.processed_source_seq + 1,v_source_seq,v_head.current_revision,v_head.privacy_epoch,v_head.mode_epoch,p_command_id
