...
Status: **the full hosted prototype journey passed on September 26, 2026, including automatic memory, fresh-chat recall, customer birth setup and live Atros use. Repeated semantic acceptance remains pending.** See the [pipeline repair receipt](../../qa/2026-09-26-pipeline-repair.md) and [visual comparison](../../qa/2026-09-26-visual-comparison.md). Earlier September 23 evidence below remains historical, not the current hosted blocker.
...
The test-only [`p3-evaluator.ts`](../../../tests/person-model-v3/p3-evaluator.ts) adapter checks the existing `PersonObjectPayloadSchema`, required object kinds, source turn identity, exact character spans, speaker/subject match, direct-reported evidence role, listed hard-unknown values, and exact-string forbidden phrase probes. The corpus suite also runs all 20 histories through the current `ConsolidationSourceSchema`, `PersonExtractionOutputSchema`, `PersonObservationDraftSchema`, and `validateObservationSpans` seams, while explicitly keeping assistant turns out of direct-user observations. The same command now includes [`person-consolidation.recovery.test.ts`](../../../src/workflows/person-consolidation.recovery.test.ts), a provider-free workflow seam that fails compose or verify exactly once, leaves the prior revision untouched, then proves one candidate, one publication, one source outcome, checkpoint reuse, and a no-op replay. This is still a **mechanical contract evaluator**, not a semantic-quality score: exact spans do not establish relevance, mocked recovery does not prove hosted scheduling, and phrase checks do not detect paraphrased hallucinations. The durable worker now uses these typed seams, but the corpus has not yet been executed 3× per case through configured providers.
...
The local optimized-production run passed with real provider calls. It must not be replaced by the P2 visual test's direct trusted-output fixture. That fixture now adds a narrower interaction receipt: it forces one rejection API failure, verifies that the UI does not claim success, retries through the actual typed-change endpoint, and checks the resulting `person_changes` plus message-less `person_source_items` rows. The worker now reconstructs correction text from the immutable change request, filters excluded evidence, and keeps source ranges contiguous. The deterministic workflow suite separately proves fail-once compose/verifier recovery without provider spend. These tests still do not prove provider-backed correction publication or hosted scheduler recovery; both remain required before full P3 acceptance.

### Hosted staging variant (explicit opt-in; do not use for production)

The same spec has a separate fail-closed config, `playwright.p3-staging.config.ts`. The release workflow supplies the exact Preview deployment URL returned by Vercel for the current `main` commit; the config permits only an HTTPS `*.vercel.app` URL and the exact staging Supabase ref `wtloawiwntyjiidjbmuk`. It rejects production aliases and mismatched URL/ref combinations before creating a user. Both Supabase access from the runner and Chromium are pinned to the explicitly supplied staging IPv4 address. The Vercel Preview environment must itself use this same staging project and a working provider key; the browser run exercises that runtime configuration. The runner also requires `P3_STAGING_CRON_SECRET` so bounded retry dispatches remain authenticated.

Required GitHub Secrets for the release workflow: `SUPABASE_ACCESS_TOKEN`, `VERCEL_TOKEN`, `P3_STAGING_SUPABASE_REF`, `P3_STAGING_SUPABASE_URL`, `P3_STAGING_SUPABASE_IP`, `P3_STAGING_SUPABASE_SECRET_KEY`, `P3_STAGING_SUPABASE_PUBLISHABLE_KEY`, `P3_STAGING_CRON_SECRET`, and `VERCEL_AUTOMATION_BYPASS_SECRET`. The schema checker targets production project ref `ezanfqbewuqttatrkvhf` through the Supabase CLI Management API and only performs reads. It fails on missing or unrecognized migration versions or missing contract objects; it does not apply migrations. Runtime provider credentials belong in the Vercel Preview environment. Normal Preview defaults share production accounts; the test candidate explicitly overrides the database and recovery secret at build and runtime to use the isolated staging project. Provider calls may incur cost. This variant creates a disposable confirmed Auth user and sends actual messages, then signs out and deletes only that exact test user. Never point the hosted browser variables at production.
...
The authorized hosted run reached real visible authentication, foreground answer completion, source acceptance and extraction on the previous branch Preview. It first exposed an `agent_step_limit` failure; on rerun, OpenCode Go returned `429 GoUsageLimitError` for its five-hour allowance during compose. The test was stopped before further retry dispatch, disposable data was removed, and no successful hosted publication or screenshot pass was claimed at that time.

The current production release workflow runs [`system-pipeline.spec.ts`](../../../tests/e2e/system-pipeline.spec.ts) against the exact Preview candidate for the current commit. It stores desktop, laptop, and mobile screenshots as GitHub artifacts. The test captures an authenticated trace only after visible sign-in and saves it in the runner workspace; the workflow does not upload trace archives because they can contain session cookies and Vercel bypass headers.
