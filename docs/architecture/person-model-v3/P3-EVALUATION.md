...
The local optimized-production run passed with real provider calls. It must not be replaced by the P2 visual test's direct trusted-output fixture. The test still does not inject a deterministic worker failure; add a fail-once integration seam and capture last-valid-view/recoverable UI before full P3 acceptance.
